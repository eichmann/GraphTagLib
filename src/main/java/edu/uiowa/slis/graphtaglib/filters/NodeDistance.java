...
public class NodeDistance extends TagSupport {
	...
	try {
	    ...
	    // Get co-authors for node and add edge to graph only if that neighbor is within our radius

		//   we have uri -> coauthors.uri -> site 
//		PreparedStatement theStmt = conn.prepareStatement("select candidate.*,person.uri from vivo_aggregated.person, vivo_aggregated.site_loc as candidate, vivo_aggregated.site_loc as center where center.id=person.id and center.id=? and circle(center.position,(?/111.0)) @> candidate.position");
		PreparedStatement theStmt = conn.prepareStatement("select candidate.* from vivo_aggregated.site_loc as candidate, vivo_aggregated.site_loc as center where center.id=? and circle(center.position,(?/111.0)) @> candidate.position");
